...
static VALUE cursor_execute2(VALUE args)
{
	 ...
	 * ---------------------------------------------------------------- */
		...
		isc_dsql_execute2(fb_connection->isc_status,
		                  ...
		                  fb_cursor->o_sqlda);
		execute_sqlcode = isc_sqlcode(fb_connection->isc_status);
		if (execute_sqlcode != SQLCODE_NOMORE) {
			fb_error_check(fb_connection->isc_status);
		}

		if (execute_sqlcode == SQLCODE_NOMORE) {
			/*
			 * Firebird 5 can return SQLCODE 100 for DML ... RETURNING when no rows
			 * match the WHERE clause. In this state, querying SQL info may raise
			 * "attempt to fetch past the last record". Treat as a clean no-row
			 * result: zero affected rows and empty RETURNING.
			 */
			rows_affected = 0;
