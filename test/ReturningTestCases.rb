...
class ReturningTestCases < FbTestCase
  ...
  def test_returning_with_autoincrement
    skip 'Autoincrement behavior varies by Firebird version'

    sql_schema = 'CREATE TABLE TEST_RETURNING (ID INTEGER GENERATED BY DEFAULT AS IDENTITY, NAME VARCHAR(20))'
    sql_insert = 'INSERT INTO TEST_RETURNING (NAME) VALUES (?) RETURNING ID'

    Database.create(@parms) do |connection|
      connection.execute(sql_schema)

      result1 = connection.execute(sql_insert, 'First')
      id1 = result1[:returning][0]

      result2 = connection.execute(sql_insert, 'Second')
      id2 = result2[:returning][0]

      assert id2 > id1, 'IDs should be auto-incremented'
    end
  end

  def test_execute_method_returning
    sql_schema = 'CREATE TABLE TEST_RETURNING (ID INT, NAME VARCHAR(20))'
    sql_insert = 'INSERT INTO TEST_RETURNING (NAME) VALUES (?) RETURNING ID'
    ...
    Database.create(@parms) do |connection|
      ...
      result = connection.execute(sql_insert, 'Test')
      assert_instance_of Hash, result
      assert result.key?(:returning)
      assert result.key?(:rows_affected)
    end
  end

  def test_query_method_with_returning
    sql_schema = 'CREATE TABLE TEST_RETURNING (ID INT, NAME VARCHAR(20))'
    sql_insert = 'INSERT INTO TEST_RETURNING (ID, NAME) VALUES (?, ?)'

    Database.create(@parms) do |connection|
      connection.execute(sql_schema)
      connection.execute(sql_insert, 1, 'Test')

      result = connection.query(sql_insert, 2, 'Test2')
